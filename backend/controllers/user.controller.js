...
export const getFriendsUsers = async (req, res) => {
	try {
		...
		const friendsUsers = users.filter((user) => usersFollowedByMe.following.includes(user._id));
		friendsUsers.forEach((user) => (user.password = null));

		res.status(200).json(friendsUsers);
	} catch (error) {
		console.log("Error in getFriendsUsers: ", error.message);
		res.status(500).json({ error: error.message });
	}
};
